...
- **Multi-turn**: the Messages API is stateless but Claude Code sessions aren't. yagami hashes each conversation prefix and remembers which session produced it; a follow-up request resumes (forking) that session and sends only the new user message. Unmatched histories fall back to replaying the transcript in a single prompt, and if a cached session turns out to be gone (e.g. cleaned up by Claude Code), the stale mapping is dropped and the request transparently retries via transcript replay. The cache persists across restarts at `~/.config/yagami/sessions.json`.
